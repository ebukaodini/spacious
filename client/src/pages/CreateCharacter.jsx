...
import { createCharacter, characters as charactersQuery, planets as planetsQuery } from "../store";
...
export const CreateCharacterPage = () => {
  ...
  const [createCharacterFunc, { loading, error }] = useMutation(createCharacter(), {
    refetchQueries: [
      charactersQuery(),
      planetsQuery()
    ]
  });
  const [loadedPlanet, setLoadedPlanet] = useState(false)
  ...
  useEffect(() => {
    ...
    setForm({ ...form, planet: state?.planet })
    setLoadedPlanet(true)
  }, [form, loadedPlanet, state?.planet])

  return (
    <Modal>
      <ModalWrapper>

        <CloseModalBtn type="button" disabled={loading} onClick={() => goBack()}>
          <CloseModalIcon>
            <X size={32} />
          </CloseModalIcon>
        </CloseModalBtn>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <ModalInputWrapper>
            ...
            <InputWrapper>
              <InputLabel>Planet Code</InputLabel>
              <Input type='text' value={state?.planet ?? ''} disabled={loading} required onChange={(e) => setForm({ ...form, planet: e.target.value })} />
              <InputDescription>
                <span>The planet the character lives on</span>
              </InputDescription>
            </InputWrapper>
            ...
            <InputWrapper>
              ...
              <InputDescription>
                ...
                <a href={`https://www.shutterstock.com/search/${form.name || 'characters'}?orientation=horizontal&image_type=photo`} target='_blank' rel='noreferrer'>Check here.</a>
              </InputDescription>
            </InputWrapper>
            ...
            <InputWrapper>
              ...
              <Input type='text' disabled={loading} maxLength={300} onChange={(e) => setForm({ ...form, friends: e.target.value })} />
            </InputWrapper>
          ...
          <ModalActionBtns>
            ...
            <CreateBtn disabled={loading} type="submit">{loading ? 'CREATING CHARACTER ...' : 'CREATE CHARACTER'}</CreateBtn>
          </ModalActionBtns>
